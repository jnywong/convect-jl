...
nn = 4 # no. of Fourier modes (excluding 0)
a = 5 # L/D aspect ratio
Ra = 5000 # Rayleigh number
Pr = 1 # Prandtl number
...
while m<=nt
    for k=2:1:nz-1
        for n=1:1:nn
            ...
            dtemdt[k,n,2] = (n*pi/a)*psi[k,n] + (dtemdz2[k,n]-(n*pi/a)^2*tem[k,n,2]) # (3.3)
    ...
    for n=1:1:nn
        psi[:,n] = myroutines.poisson(omg[:,n,2],nz,dz,n,a) # (3.5)
    end

    # Diagnostics
    println(log(abs(tem[round.(Int,nz/3),1,2]))-log(abs(tem[round.(Int,nz/3),1,1])))
...
nx = a*nz # no. of horizontal gridpoints
dx = a/(nx-1)
x = LinRange(0,a,nx)
